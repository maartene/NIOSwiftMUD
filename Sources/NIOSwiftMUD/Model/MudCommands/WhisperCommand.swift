struct WhisperCommand: MudCommand {
    ...
    func execute(in world: World) async -> [MudResponse] {
        ...
        guard let targetPlayer = await world.userRepository.filter(where: {$0.username.uppercased() == targetPlayerName.uppercased()}).first else {
        ...
        let playersInRoom = await world.userRepository.filter(where: {$0.currentRoomID == player.currentRoomID})
